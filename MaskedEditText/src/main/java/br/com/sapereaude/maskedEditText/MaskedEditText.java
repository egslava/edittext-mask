...
public class MaskedEditText extends AppCompatEditText implements TextWatcher {
    ...
    private char passwordRepresentation;
    ...
    public MaskedEditText(Context context, AttributeSet attrs) {
        ...
        passwordMode = attributes.getBoolean(R.styleable.MaskedEditText_password_mode, false);
        ...
        String pwRepresentation = attributes.getString(R.styleable.MaskedEditText_password_representation);

        if (pwRepresentation == null) {
            passwordRepresentation = DOT;
        } else {
            passwordRepresentation = pwRepresentation.charAt(0);
        }
    ...
    private String makeMaskedText() {
        ...
        for (int i = 0; i < maskedText.length; i++) {
            ...
            if (rawIndex == -1) {
                ...
                if (passwordMode) {
                    maskedText[i] = passwordRepresentation;
                } else {
        ...
        for (int i = 0; i < mask.length(); i++) {
            ...
            if (mtrv != -1) {
                if (mtrv < rawText.length()) {
                    if (passwordMode) {
                        ssb.append(passwordRepresentation);
                    } else {
